...
const SYSTEM_PROMPT = `You are INRT AI Assistant — a smart, friendly helper built into the INRT Wallet payment app.
...
- Ticket bookings (movies, trains, buses, flights)
- Troubleshooting payment errors
- Understanding charges and limits

When users report errors, diagnose them clearly and give step-by-step fixes.
Keep answers short, friendly, and helpful. Use emojis occasionally.
Always prioritize user safety — never ask for PINs or passwords.
If asked about something outside payments/finance, politely redirect.`;
...
export default function AIAssistant({ onClose }: { onClose: () => void }) {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: 'assistant',
      content: `Hi ${userProfile?.name?.split(' ')[0] || 'there'}! 👋 I'm your INRT AI Assistant. I can help you with payments, KYC, rewards, bookings, and fix any errors you face. What can I help you with today?`
    }
  ]);
      ...
      <div style={s.panel}>
        ...
        <div style={s.header}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              <p style={s.name}>INRT AI Assistant</p>
              <p style={s.online}>● Online · Always available</p>
            </div>
          </div>
...
const s: Record<string, React.CSSProperties> = {
  ...
  botBubble: { background: '#f1f5f9', borderRadius: '18px 18px 18px 4px', padding: '12px 16px', maxWidth: '78%', fontSize: 14, color: '#111', lineHeight: 1.5 },
  userBubble: { background: 'linear-gradient(135deg, #00b9f1, #0090c0)', borderRadius: '18px 18px 4px 18px', padding: '12px 16px', maxWidth: '78%', fontSize: 14, color: '#fff', lineHeight: 1.5 },
