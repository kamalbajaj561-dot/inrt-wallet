 ...
 * Replaces: src/pages/Dashboard.tsx
 */
...
// ── Design tokens (INRTPay V2 style) ─────────────────────────
const T = {
  navy:    '#0A2540',
  accent:  '#0070F3',
  inrt:    '#7B2FBE',
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const u2 = subscribeToTransactions(user.uid, t => setTxns(t.slice(0, 5)));
    return () => { u1(); u2(); };
  }, [user]);
      ...
      <div style={{ background: `linear-gradient(145deg,${T.navy} 0%,#1565C0 100%)`, padding: '22px 20px 80px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 22, position: 'relative' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              ...
              <p style={{ color: '#fff', fontSize: 16, fontWeight: 800, margin: 0, fontFamily: "'Plus Jakarta Sans',sans-serif" }}>
                {name.split(' ')[0]}
              </p>
            </div>
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            <button
              ...
              style={{ background: 'rgba(255,255,255,0.1)', border: 'none', borderRadius: 10, width: 38, height: 38, color: '#fff', cursor: 'pointer', fontSize: 16 }}>
              📷
            </button>
            <button
        ...
        {kyc !== 'verified' && (
          <button
            ...
            style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%', background: 'rgba(255,214,10,0.12)', border: '1px solid rgba(255,214,10,0.3)', borderRadius: 12, padding: '9px 12px', marginBottom: 14, cursor: 'pointer', position: 'relative' }}>
            <span style={{ fontSize: 14 }}>⚠️</span>
            <span style={{ color: T.gold, fontSize: 12, fontWeight: 600, flex: 1, textAlign: 'left' }}>
        ...
        <div style={{ display: 'flex', gap: 10, position: 'relative' }}>
          ...
          <div
            ...
            <p style={{ color: 'rgba(224,176,255,0.45)', fontSize: 11, margin: 0 }}>
              1 INRT = ₹1 · Stablecoin 🔒
            </p>
          </div>
        </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 6 }}>
            {ACTIONS.map(a => (
              <button key={a.path} onClick={() => navigate(a.path)}
                ...
                <span style={{ fontSize: 11, color: T.text, fontWeight: 700, fontFamily: "'Plus Jakarta Sans',sans-serif", textAlign: 'center', lineHeight: 1.3 }}>
                  {a.label}
                </span>
              </button>
            ))}
      ...
      <div style={{ padding: '16px 16px 0' }}>
        ...
        <div
          ...
          <div>
            ...
            <p style={{ color: '#fff', fontSize: 17, fontWeight: 800, margin: '8px 0 4px', fontFamily: "'Plus Jakarta Sans',sans-serif" }}>
              Pay in Bali or Spain 🌍
            </p>
            <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: 12, margin: 0 }}>
              Zero forex charges with INRT
            </p>
          </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          <h3 style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontWeight: 800, fontSize: 16, color: T.text, margin: 0 }}>All Services</h3>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, marginBottom: 16 }}>
          {[
            ...
            <button key={s.path} onClick={() => navigate(s.path)}
              ...
              <span style={{ fontSize: 10, color: T.text, fontWeight: 700, fontFamily: "'Plus Jakarta Sans',sans-serif", textAlign: 'center' }}>
                {s.label}
              </span>
            </button>
          ))}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          ...
          <button onClick={() => navigate('/history')}
            style={{ color: T.accent, background: 'none', border: 'none', cursor: 'pointer', fontSize: 13, fontWeight: 700 }}>
            View all →
          </button>
        </div>
      ...
      <nav style={{ position: 'fixed', bottom: 0, left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: 480, height: 72, background: 'rgba(255,255,255,0.97)', borderTop: `1px solid ${T.border}`, display: 'flex', alignItems: 'center', zIndex: 200, padding: '0 8px 8px', backdropFilter: 'blur(20px)' }}>
        {[
          ...
          <button key={t.path} onClick={() => navigate(t.path)}
            style={{ flex: 1, display: 'flex', flexDirection: 'column' as const, alignItems: 'center', gap: t.special ? 0 : 3, cursor: 'pointer', background: 'none', border: 'none', padding: '6px 0', fontFamily: "'Plus Jakarta Sans',sans-serif" }}>
            {t.special ? (
              <div style={{ width: 44, height: 44, borderRadius: '50%', background: `linear-gradient(135deg,${T.teal},#00b4a0)`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#000', fontSize: 24, fontWeight: 900, boxShadow: '0 4px 16px rgba(0,229,204,0.4)', marginTop: -8 }}>
                +
              </div>
            ) : (
              <>
